...
datadir <- '../data'
alldata <- read.csv(file.path(datadir, 'food-dmnd-price-allrgn.csv'))
xval.rgn.trn <- read.csv(file.path(datadir, 'xval-byrgn-trn.csv'))
xval.rgn.tst <- read.csv(file.path(datadir, 'xval-byrgn-tst.csv'))
xval.yr.trn <- read.csv(file.path(datadir, 'xval-byyear-trn.csv'))
xval.yr.tst <- read.csv(file.path(datadir, 'xval-byyear-tst.csv'))

## read the mc results:  we assume that the data has all been gzipped
mcdir <- '../runs'
mcrslt.all <- read.mc.data(file.path(mcdir, 'mc-food-dmnd.allrgn.dat.gz'))
mcrslt.rgn <- read.mc.data(file.path(mcdir, 'mc-food-dmnd.xval-byrgn.dat.gz'))
mcrslt.yr <- read.mc.data(file.path(mcdir, 'mc-food-dmnd.xval-byyear.dat.gz'))
```

## Maximum Likelihood Parameter Sets
```{r ML}
pv.all <- mcparam.ML(mcrslt.all)        # vector for passing to mc.likelihood 
p.all <- vec2param(pv.all)               # parameter struct for passing to food.dmnd
pp.all <- mcrslt.all[which.max(mcrslt.all$LL),] # data frame for pretty-printing
 
pv.rgn <- mcparam.ML(mcrslt.rgn)
p.rgn <- vec2param(pv.rgn)
pp.rgn <- mcrslt.rgn[which.max(mcrslt.rgn$LL),] # data frame for pretty-printing
 
pv.yr <- mcparam.ML(mcrslt.yr)
p.yr <- vec2param(pv.yr)
...
## Alternate Figure 4
This version focuses on two regions where the model does well (before bias correction) and two where
it doesn't do so well.
```{r fig4a}
print(plts.mc.all$byyear.focus)
```
## Figure 5
```{r fig5}
...
## Alternate Figure 6
This version of figure 6 shows only the residuals from the cross-validation by year  This is a better fit for talks where 
we don't have time to go into the cross-validation by region or why it didn't turn out to be useful.

```{r fig6a}
print(resid.stats$scatter.xval.byyr)
